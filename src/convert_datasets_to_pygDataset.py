...
def load_dataset(path='../data/raw_data/', dataset='mimic3',
    ...
    # encode text
    if text:
        text_features = encode_bert(dataset=dataset, text_dim=features.shape[1])
        features = torch.hstack((text_features, features))
